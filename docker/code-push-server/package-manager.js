...
proto.releasePackage = function (appId, deploymentId, packageInfo, filePath, releaseUid) {
    ...
    .then((type) => {
      return models.Apps.findById(appId).then((appInfo)=>{
        if (type > 0 && appInfo.os > 0 && appInfo.os != type) {
          ...
          thisAppInfo = {os : appInfo.os, name: appInfo.name};
          //不验证
  ...
  .then((blobHash) => {
    return security.getUploadPackageBundle(directoryPath)
      .then((bundlePath) => {
        return new Promise((resolve, reject) => {
          const data = fs.readFileSync(bundlePath, 'utf8');
          const regex_ios = /(?<=window\.CodePushAppNameIOS=")[^"]+(?=")/gmi;
          const regex_android = /(?<=window\.CodePushAppNameAndroid=")[^"]+(?=")/gmi;
          const regex = (thisAppInfo.os == 1) ? regex_ios : regex_android; //ios 1 android 2
          const appName = thisAppInfo.name;
          const matchResult = data.match(regex);
          const codePushTargetInBundle =  matchResult && matchResult.length > 0 ? matchResult[0] : '';
          if (!codePushTargetInBundle || (codePushTargetInBundle !== appName)) {
            var e = new AppError.AppError(`CodePushAppName 設置不正確  預期 [${appName}] 實際 [${codePushTargetInBundle}]，請檢查是否推錯代碼？`);
            log.debug(e.message);
            throw e;
          }
          resolve(blobHash);
